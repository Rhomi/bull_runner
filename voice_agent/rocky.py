...
def main():
    ...
    valid_query = False
    if  sys.argv[1] or sys.argv[2] == 'and': # gotta improve this logic
        valid_query = True

        query = sys.argv[1:]
        split_point = query.index('and')
        # for route names such as 'c','f'
        if len(query[:split_point]) == 1:
            route = ''.join(query[:split_point])
       	# for route names such as 'msc express' and 'library express' -- add a space in between while joining
        else:
            route = ' '.join(query[:split_point])
        stop = ' '.join(query[split_point+1:])
    else:
        final_message = "Oops. I can only understand it when you separate the route name and stop name with the word 'and'. For example, c and sundome west"     
        say_it(final_message)

    if(valid_query):
        route,stop = preprocessing(route,stop)
